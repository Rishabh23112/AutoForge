...
from agents.prompt import analysis_prompt, analysis_parser, FileRelevance
...
def get_relevant_files(state: ProjectState, instruction: str) -> List[str]:
    file_list = "\n".join([f.path for f in state.files])
    ...
    structured_llm = llm.with_structured_output(FileRelevance)
    
    try:
        response = structured_llm.invoke(formatted_prompt)
        return response.relevant_files
    except Exception:
        # Fallback to all files if analysis fails
        return [f.path for f in state.files]


def modify_project_state(state: ProjectState, instruction: str) -> ProjectState:
    relevant_paths = get_relevant_files(state, instruction)
    
 
    formatted_files = ""
    for file in state.files:
        if file.path in relevant_paths:
            formatted_files += f"FILE: {file.path} (FULL CONTENT)\n{file.content}\n\n"
        else:
            formatted_files += f"FILE: {file.path} (EXISTING - SKIPPING CONTENT)\n\n"
